<div class="video_upload content_upload" style="display:none" id="{{ $editStatus }}content-zip">
    <div class="row justify-content-center">
        <div class="col-10">
            <div class="form-group">
                ...
                <input class="form-control col-9 form-con" id="{{ $editStatus }}task-title-zip" type="text" placeholder=""
                ...
                <div class="video_select col-9 p-0">
                    ...
                    <div class="data_select_list" style="border: 0;border-bottom: 1px solid #d5d5d5;margin-bottom: 0;-webkit-border-radius: 0;-moz-border-radius: 0;border-radius: 0;">
                        <div class="data_select_item">
                            ...
                            已上传文件：{{ render_task_source($task->target['media_uri']) }}
                            </span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active _upload" id="home" role="tabpanel" aria-labelledby="home-tab">
                            ...
                            @include('vendor.progress.progress', ['editStatus' => $editStatus, 'type' => 'zip'])
                            ...
                            <div class="file_upload" style="overflow: hidden;">
                                <input type="file" name="zip" id="{{ $editStatus }}zip-up" accept="application/x-zip-compressed,application/octet-stream"
                                       class="custom-input-file custom-input-file--2"
                                       data-token="{{ route('manage.qiniu.token.hash') }}"/>
                                <label for="{{ $editStatus }}zip-up" style="margin: 93px 50px 0;">
                                    <i class="iconfont" style="float:left;font-size:25px;">&#xe658;</i>
                                    <span id="file_name" style="display:block;margin-top: 5px;">请选择压缩包</span>
                                    <input type="hidden" class="form-con" id="{{ $editStatus }}file-zip-key" value="{{ !empty($task) && $task->target ? $task->target->media_uri : '' }}">
                                    <input type="hidden" class="form-con" id="{{ $editStatus }}file-zip-hash" value="{{ !empty($task) && $task->target ? $task->target->hash : '' }}">
                                </label>
                            </div>
                        </div>
        ...
        {{--<div class="col-10">--}}
            {{--<div class="form-group">--}}
                {{--<label class="form-control-label col-2">--}}
                    ...
                    {{--视频时长--}}
                {{--</label>--}}
                {{--<input class="form-control col-6" id="task-length-video" type="text" placeholder="">--}}
