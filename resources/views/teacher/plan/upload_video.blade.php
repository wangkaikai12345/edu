<div class="video_upload content_upload" style="display:none" id="{{ $editStatus }}content-video">
    <div class="row justify-content-center">
        <div class="col-10">
            <div class="form-group">
                ...
                <input class="form-control col-9 form-con" id="{{ $editStatus }}task-title-video" type="text" placeholder=""
                ...
                <div class="video_select col-9 p-0">
                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active _upload" id="home" role="tabpanel" aria-labelledby="home-tab">
                            ...
                            @include('vendor.progress.progress', ['editStatus' => $editStatus, 'type' => 'video'])
                            ...
                            <div class="file_upload" style="overflow: hidden;">
                                <input type="file" name="video" id="{{ $editStatus }}video-up" accept="video/*"
                                       class="custom-input-file custom-input-file--2"
                                       data-token="{{ route('manage.qiniu.token.hash') }}"/>
                                <label for="{{ $editStatus }}video-up" style="margin: 93px 50px 0;">
                                    <i class="iconfont" style="float:left;font-size:25px;">&#xe658;</i>
                                    <span id="file_name" style="display:block;margin-top: 5px;">请选择视频</span>
                                    <input type="hidden" class="form-con" id="{{ $editStatus }}file-video-key" value="{{ !empty($task) && $task->target ? $task->target->media_uri : '' }}">
                                    <input type="hidden" class="form-con" id="{{ $editStatus }}file-video-hash" value="{{ !empty($task) && $task->target ? $task->target->hash : '' }}">
                                </label>
                            </div>
                        </div>
