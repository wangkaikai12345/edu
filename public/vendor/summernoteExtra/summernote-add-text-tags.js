...
(function (factory) {
    ...
    $.extend($.summernote.plugins, {
        ...
        'add-text-tags': function (context) {
            ...
            var del = self.generateBtn('del', '删除文字');
            var ins = self.generateBtn('ins', '插入文字');
            var small = self.generateBtn('small', '小字体');
            var mark = self.generateBtn('mark', '高亮文字');
            var variable = self.generateBtn('var', '变量');
            var keyboard = self.generateBtn('kbd', '用户输入');
            var code = self.generateBtn('code', '行内代码');
            var samp = self.generateBtn('samp', '样例输出');
